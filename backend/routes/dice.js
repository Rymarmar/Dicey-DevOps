...
function payoutFrom(prob, didWin, wager) {
  ...
  return { profit, fairMult: +fairMult.toFixed(2), paidMult: +paidMult.toFixed(2) };
}

// ---------- routes ----------

// Simple demo roll (kept from your MVP)
r.get("/roll", (_req, res) => {
  const dice = roll2();
...
// Main game endpoint
// Body: { betType: "sum"|"exactPair"|"anyDoubles"|"overUnder7",
//         selection: number | [n,n] | "over"|"under"|"seven",
//         wager: number }
r.post("/play", (req, res) => {
  ...
  res.json({
    dice, total, win,
    payout: profit,                 // positive if win, negative if loss
    prob: +p.toFixed(4),
    fairPayoutMultiplier: fairMult, // “X:1” style profit per $1
